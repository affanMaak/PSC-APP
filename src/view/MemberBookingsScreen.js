...
export default function MemberBookingsScreen({ navigation }) {
    ...
    const handleCancelBooking = async (item) => {
        ...
        if (item.checkIn) {
            ...
            // but usually check-in date being today or in past is enough to block.
            if (checkInDate < now.setHours(0, 0, 0, 0)) {
                Alert.alert('Cannot Cancel', 'This booking cannot be cancelled because the check-in date has already passed.');
                return;
            }
        }
    ...
    const submitCancellationRequest = async () => {
        ...
        const bookingType = selectedBooking.bookingType || 'Room';
